...
> 文档用途：交给负责编码、测试、审查和桌面端接入的智能体作为统一实施依据。
>
...
- T8：组件目录、兼容性服务、数据集校验/预览、本地 FastAPI 骨架和可取消任务状态机已实现。

移除旧兼容测试并增加新数据生成器测试后，当前自动化测试结果为 `38 passed`。桌面 UI、SQLite 工作区持久化、训练独立进程、实时推理和远程 CI 属于下一阶段产品建设，不属于本次数据管线重构的完成条件。
...
本次重构的首要目标不是增加功能，而是建立稳定、可测试、可由桌面端驱动的运行时契约。

### 1.1 必须达成的目标

1. 只保留一个核心 `SERDataset`，Dataset 不再按 waveform、spectrogram、feature 分类。
2. 将数据来源、音频加载、增强、输入表示、批处理完全解耦。
3. 所有训练、验证、测试、离线推理和桌面预览复用相同的 `AudioLoader` 与 `Representation`。
...
7. 为桌面端提供可枚举、可校验、可生成表单的组件描述信息。
8. 建立足够测试，使后续模型层和桌面端不依赖实现细节。
...
配置文件中的相对路径目前可能相对于进程启动目录解析。桌面应用、Notebook、CLI 和测试的当前工作目录不同，会造成同一配置在不同入口下表现不同。

目标规则：

- manifest 路径相对于配置文件所在目录解析；
- manifest 内部音频相对路径相对于 manifest 声明的 `root` 解析；
- 工作区内部尽量保存相对路径，外部引用保存规范化绝对路径；
- 不允许依赖 `cwd` 作为隐式业务路径。
...
Mixup、RIR、动态噪声等存在可配置但运行时抛 `NotImplementedError` 的情况。新注册表不得发布不可运行组件。实验性组件必须声明状态，并且默认不出现在桌面端普通组件列表中。
...
4. **模型显式声明输入要求。** 禁止前端或训练器猜测 tensor shape。
...
9. **导入不复制大文件。** 默认引用原始音频；复制到工作区必须由用户显式选择。
10. **公共接口不泄露内部配置细节。** 桌面端只依赖 descriptor 和服务 API。
...
仓库根目录中存放真实语料的 `data/` 后续建议更名为 `datasets/` 或由桌面工作区管理，但本次不要自动移动用户数据。
...
必须区分 `scan()` 和 `convert()`：桌面端先预览解析结果、标签映射和错误，再由用户确认写入工作区。
...
Pydantic 配置类的 JSON Schema 可以作为前端表单来源，但前端不得假设所有 JSON Schema 特性都受支持。第一版限制到数字、字符串、布尔、枚举、数组和简单嵌套对象。
...
数据预处理工具直接生成标准 `dataset.yaml`、JSONL 和新的 `DataConfig` 文件。所有代码、测试、教程和未来桌面端只能使用 `ser_lib.data`。
...
- 不在库代码中打印；使用 logging，并允许桌面端接收结构化事件。
- 不在 Dataset worker 中更新 SQLite。
...
桌面应用采用 Web UI 套壳时，数据模块需要提供服务层，而不是让前端拼装 Python 内部对象。

建议服务接口：

```text
GET  /api/components/importers
GET  /api/components/representations
GET  /api/components/transforms
GET  /api/components/models
...
- 扫描、导入、缓存生成和训练都是可取消后台任务。
- API 返回工作区资源 ID，不要求前端长期保存任意绝对路径。
- 音频预览接口必须限制可访问范围，不能成为任意本地文件读取接口。
- 前端表单由 descriptor schema 生成，但后端始终重新校验。
- 日志和进度使用 SSE/WebSocket；任务结果持久化到 SQLite 和工作区文件。
- 关闭 UI 不应隐式终止训练；退出桌面应用时由任务管理器明确处理。
...
- 桌面端能通过 descriptor 枚举表示与参数，并在创建任务前执行兼容性检查。
...
- [ ] 是否让桌面前端承担后端应执行的兼容性校验。

---

本文档的核心决策是：**数据集只描述样本集合，Representation 描述输入形式，TensorSpec 描述形状契约，Collator 根据规格批处理。** 所有后续模型、训练、推理和桌面端能力都应建立在这四个边界之上。
